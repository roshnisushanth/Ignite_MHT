...
var renderTable = function (success, error) {
    ...
    $.ajax({
        type: "POST",
        url: "HickChatEngine.svc/GetAudit_log",
        data: '{"currentuserid":' + userid + ',"usertype":"' + _userType + '","peerid":' + _peerid + ',"PageIndex":"0","PageSize":"1"}',
        contentType: "application/json; charset=utf-8",
        success: function (msg) {
            var data = msg.d,
            audits = data.AuditColl,
            auditHeader = getHeaderTemplate(),
            auditRow = "", adt;

            for (var i = 0; i < audits.length; i++) {
                adt = audits[i];

                auditRow += i % 2 == 0 ? '<div class="row" style="background-color:lavenderblush;">' : '<div class="row">';

                auditRow += '<div class="col-xs-6 col-sm-2 col-lg-2">' + adt.ActionType + '</div>' +
                    '<div class="col-xs-6 col-sm-2 col-lg-2">' + adt.InformationType + '</div>' +
	                //'<div class="col-xs-6 col-sm-2 col-lg-2">' + adt.OldValue + '</div>' +
	                //'<div class="col-xs-6 col-sm-2 col-lg-2">' + adt.NewValue + '</div>' +
	                '<div class="col-xs-6 col-sm-2 col-lg-2">' + adt.Date + '</div>' +
	                '<div class="col-xs-6 col-sm-2 col-lg-2">' + adt.Email + '</div>' +
                  '</div> ';
            }
            $('#auditlist').html(auditHeader + auditRow);
            stopEventsTimer();
        },
        error: function (err) {
            //alert(err);
    ...
    if (groupcoll) {
        if (groupcoll.length > 0) {
            for (var i = 0; i < groupcoll.length; i++) {
                ...
                template = "<tr id='tableRow' data-row-key='${User}' class='tableRow " + clsgrp + "' style='cursor:pointer;' groupid=" + groupcoll[i].Id + "> <td id='userThumbnail'><img id='imgOption' src='Images/group_icon.png' style='width:50px; height:50px;'></img><img id='status' src='Images/option_unselected.png'></img></td> <td valign='top'><span id='" + groupcoll[i].Id + "' class='userselct grpusers' >" + trimmed_userlist + "</span><div class='small1' id='statusmessage'></div></td> <td></td> <td></td></tr>";
...
function getHeaderTemplate() {
    return '<div class="row" style="background-color:lavender;">' +
                '<div class="col-xs-6 col-sm-2 col-lg-2"><b>Action Type</b></div>' +
                '<div class="col-xs-6 col-sm-2 col-lg-2"><b>Information Type</b></div>' +
	            //'<div class="col-xs-6 col-sm-2 col-lg-2"><b>Old Value</b></div>' +
	            //'<div class="col-xs-6 col-sm-2 col-lg-2"><b>New Value</b></div>' +
	            '<div class="col-xs-6 col-sm-2 col-lg-2"><b>Action Date</b></div>' +
	            '<div class="col-xs-6 col-sm-2 col-lg-2"><b>Email</b></div>' +
          '</div>';
};
...
$("#morecontacts").click(function () {
    ...
    $.ajax({
        ...
        contentType: "application/json; charset=utf-8",
        success: function (msg) {
            var audits = msg.d.AuditColl,auditRow = "",adt;
            for (var i = 0; i < audits.length; i++) {
                auditRow += (i % 2 == 0) ? '<div class="row" style="background-color:lavenderblush;">' : '<div class="row">';
